...
PdfObjVal pdfArrayAtGet(PdfArray *pdfArray, int index) {
  ...
  if (index < pdfArray->size) {
    ...
    if (pdfArray->arr[index] == NULL) {
      ...
      return (PdfObjVal){.ptr = pdfArray->arr[index], .type = PDFNULL};
    } else {
      return (PdfObjVal){.ptr = pdfArray->arr[index], .type = pdfArray->values_types[index]};
    }
  } else {
    return (PdfObjVal){.ptr = pdfArray->arr[index], .type = PDFNULL};
  }
...
void pdfArrayAtSet(PdfArray *pdfArray, int index, PdfObjVal pdfObjVal) {
  ...
  if (index < pdfArray->size) {
    ...
    while (index >= pdfArray->size) {
      ...
      pdfArray->values_types = realloc(pdfArray->values_types, (pdfArray->size * 2) * (sizeof(void *)));

      // check of the reallocation failed.
...
PdfArray *makePdfArray(int32_t size) {
  ...
  FAIL_IF_NULL(pdfArray->values_types, "Failed to allocate memroy for the types sub array of the "
                                       "pdfArray object.\n");
...
void freePdfArray(PdfArray *pdfArray) {
  ...
  if (NULL != pdfArray) {
    ...
    free(pdfArray->arr);
    // free the array of integers representing the types of elements inside the
    // array.
...
PdfDictionary *makePdfDictionary(int32_t size) {
  ...
  FAIL_IF_NULL(pdfDictionary->values, "Failed to allocate memroy for pdfDictionary object values "
                                      "objects array.\n");
  ...
  FAIL_IF_NULL(pdfDictionary->values, "Failed to allocate memroy for pdfDictionary array of "
  ...
  for (int index = 0; index < pdfDictionary->size; index++) {
    ...
    if (pdfDictionary->keys[index] == NULL) {
      ...
      if (strcmp(NT_str, pdfDictionary->keys[index]->NT_str) == 0) {
        return (PdfObjVal){.ptr = pdfDictionary->values[index], .type = pdfDictionary->values_types[index]};
      }
    }
  }
  // the whole dictionary does not have a key NT-str. so return null.
  return (PdfObjVal){.ptr = NULL, .type = PDFNULL};
}
...
void pdfDictionarySet(PdfDictionary *pdfDictionary, const char *NT_str, PdfObjVal pdfObjVal) {
  ...
  if (last_free_position >= 0) {
    ...
    // assign the new element at the last open position.
  ...
  // copy the given key string to the keys array.
}
